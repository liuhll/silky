...
namespace Silky.Lms.EntityFrameworkCore.Interceptors
{
    ...
    internal sealed class SqlConnectionProfilerInterceptor : DbConnectionInterceptor
    {
        ...
        {
            // 打印连接信息消息
            // EngineContext.Current.PrintToMiniProfiler(MiniProfilerCategory, "Information", $"[Connection Id: {eventData.ConnectionId}] / [Database: {connection.Database}]{(IsPrintDbConnectionInfo ? $" / [Connection String: {connection.ConnectionString}]" : string.Empty)}");
        }
    }
}