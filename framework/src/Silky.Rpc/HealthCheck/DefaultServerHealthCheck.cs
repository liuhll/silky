...
{
    public class DefaultServerHealthCheck : IServerHealthCheck, ITransientDependency
    {
        private const string healthCheckServiceInvoke = "Silky.Rpc.AppServices.IRpcAppService.IsHealth_Get";
        ...
        {
            var serviceEntry = _serviceEntryLocator.GetServiceEntryById(healthCheckServiceInvoke);
            try
            {
        ...
        public Task<bool> IsHealth(IRpcEndpoint rpcEndpoint)
        {
            ...
            {
                return Task.FromResult<bool>(UrlCheck.UrlIsValid($"{rpcEndpoint.ServiceProtocol}://{address}",
                    out var _));
            }
