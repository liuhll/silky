    ...
    public class JwtModule : SilkyModule
    {
        ...
        protected override void RegisterServices(ContainerBuilder builder)
        {
            ...
            builder.Register(c =>
            {
                ...
                var validator = c.Resolve<IJwtValidator>();
                var urlEncoder = c.Resolve<IBase64UrlEncoder>();
                var jwtOptions = c.Resolve<IOptionsSnapshot<JwtOptions>>().Value;
                var algorithm = c.ResolveNamed<IJwtAlgorithm>(jwtOptions.Algorithm.ToString());
                var jwtDecoder = new JwtDecoder(serializer, validator, urlEncoder, algorithm);
                return jwtDecoder;
            }).InstancePerDependency().As<IJwtDecoder>();
        }
    }
}